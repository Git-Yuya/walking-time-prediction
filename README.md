...
## 連合学習の評価
　データセットを訓練データ:テストデータ=7:3でランダムに分割する。評価指標は、1mあたりの歩行時間 [s/m]の平均絶対誤差 (Mean Absolute Error; MAE)を使用し、テストデータ全体と混雑度別に分けたテストデータで評価する。評価指標のMAEは、値が0に近い方が予測精度が高いことを表している。
...
## 結論
　テストデータ全体の評価で比較すると、従来の1箇所に集約する機械学習モデルは0.0627、データセットをランダムに分割した連合学習モデルは0.0619～0.0625、データセットを歩行速度別に分割した連合学習モデルは0.0626～0.0628である。評価指標は、1mあたりの歩行時間の実測値と理論値との誤差であり、0.0001の違いは1,000m歩いた場合に0.1sの差が出ることを表している。そのため、本プロジェクトの条件下では、従来の1箇所に集約する機械学習モデルと連合学習モデルの予測精度はほとんど変わらないと分かった。
